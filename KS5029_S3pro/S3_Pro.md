...
## Key Features
- **Multi‑function Mainboard for Advanced Projects**: On‑board current‑detection circuit, SD‑card slot and programmable RGB LEDs support data logging and status indication for flexible expansion.
...
- **Expandable**: The expansion board exposes extra GPIO, I²C and other interfaces via 2.54 mm pin headers for adding more sensors or functional modules to the robot.
...
| :--- | :--- | :--- | :--- |
| Assembled chassis (no pre-wiring) | ESP32-S3-Pro Xiaozhi AI Chat Robot Pre-assembled Semi-finished Unit | Fix mainboard, easy wiring, complete assembly set for building Xiaozhi AI voice robot | ![](media/image-20260918153055962.png) |
...
### 1. ESP32‑S3‑Pro Development Board
![Img](media/KS5034.png)

The ESP32‑S3‑Pro development board is built around the ESP32‑S3‑WROOM‑1‑N16R8 module with 16 MB Flash and maximum CPU frequency of 240 MHz. It adopts pin assignment and physical form factor compatible with Arduino‑UNO series, supporting standard shields for connecting various sensors and actuators. Beyond core computing performance, this board integrates on‑board INA180 current‑sense chip, MicroSD/TF‑card mounting, USB‑OTG and programmable RGB LED. Most GPIO pins are broken‑out via male/female pin headers. On‑board DC‑DC and LDO power regulators provide stable 5 V / 3.3 V output to power common peripherals. It fits AI voice interaction, data logging and portable‑device development scenarios. RST reset and BOOT buttons are available for debugging and firmware flashing.
...
- **High‑Performance Processing**
  ...
  - 16 MB Flash storage for complex programs and resource assets
...
- **On‑Board Peripherals & Interfaces**
  - On‑board programmable RGB LED (GPIO45) for status indication
  - INA180 current‑sense IC (GPIO1) for real‑time system power‑consumption monitoring
  - MicroSD/TF‑card support (4‑wire SDIO mode) for data storage and log recording
  - USB‑OTG capability (GPIO19 / 20) for attaching external USB devices
  - Most GPIOs broken‑out via pin headers; Arduino‑UNO‑compatible layout for easy expansion

- **Power Management**
  - DC jack input: 7‑12 V for main system power supply
  - USB Type‑C provides 5 V power input and program downloading
  - On‑board DC‑DC converter outputs 5 V / 1.5 A; LDO outputs 3.3 V to feed peripherals
...
|-------------------|-------------------------|---------------------------------------------|
| **Operating Voltage**      | 3.3 V ~ 5 V           | Typically 3.3 V or 5 V                            |
...
| --- | --- |
| GPIO **20** | **SCL** (SPI Clock Signal) |
| GPIO **21** | **SDA** (SPI Data Signal) |
| GPIO **38** | **RES** (Reset Signal) |
...
2.  **Connect Hardware**: Select KS5029 preset. Connect the ESP32‑S3‑Pro kit to your PC with Type‑C cable.
...
![](media/67e3d89096b079957270155a1bb9f545.png) [**Firmware Archive**](KS5029-EN-merged-binary.bin)
